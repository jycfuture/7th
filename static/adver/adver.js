...
function AdPrepare(id, title, url, mode, pic, width, height, top, side, autoClose)
{
	...
	if (mode == "弹出广告" || mode == "Pop-up ads")
	{
		AdPopup(id, title, content, width, height, top, side, autoClose);
		return;
	}

	document.writeln("<span id='adver_" + id + "' style='z-index:99999; width:" + width + "px; height:" + height + "px; top:" + top + "px; " + (mode == "右侧门帘广告" || mode == "Ad on right" ? "right" : "left") + ":" + side + "px; cursor:pointer; position:absolute; overflow:hidden;'>" + content + "</span>");

	var obj = document.getElementById("adver_" + id);
	switch(mode)
	{
		case "Floating ads"||"漂浮广告":
			AdFloat(obj, width, height, top, side, autoClose);
			break;
		case "左侧门帘广告"||"Ad on left":
			AdHangLeft(obj, top, autoClose);
			break;
		case "右侧门帘广告"||"Ad on right":
...
function AdContent(url, pic, width, height)
{
	...
	if (picType == "swf")
	{
		...
		if (url != "")
			if (hasButton)
				content = "<span style='z-index:1;position:absolute;'><a href='" + url + "' target='_blank'><img src='static/adver/blank.gif' width='" + width + "' height='" + height + "' /></a><span style='position:absolute; top:0; right:0;' onclick='this.parentNode.style.display = \"none\";'><img src='static/adver/closed.gif' /></span></span>" + content;
			else
				content = "<span style='z-index:1;position:absolute;'><a href='" + url + "' target='_blank'><img src='static/adver/blank.gif' width='" + width + "' height='" + height + "' /></a></span>" + content;
		else
			content = "<span style='z-index:1;position:absolute;'><img src='static/adver/blank.gif' width='" + width + "' height='" + height + "' /></span>" + content;
	}
	else
	{
		...
		if (url != "")
			if (hasButton)
				content = "<a href='" + url + "' target='_blank'>" + content + "</a><span style='position:absolute; top:0; right:0;' onclick='this.parentNode.style.display = \"none\";'><img src='static/adver/closed.gif' /></span></span>";
			else
