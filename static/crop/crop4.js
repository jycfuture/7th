/**
 ...
 @ 2016-2-18
	在auhhome.com中使用，添加了回调函数，回调函数中发送到服务器裁剪图片，并更新当前显示的图片
*/
...
(function($)
{
	$.fn.crop = function (opt)
	{
		...
		var type	= _this.get(0) ? _this.get(0).tagName.toLowerCase() : 'input';
			...
			if (type == 'input') {
				_this.change(function(){
					...
					if ( window.webkitURL && window.webkitURL.createObjectURL ) {
						...
						$.fn.cropOpenWindowToCrop(ipt, fileData, width, height, opts);
					}
						...
						$.fn.cropReadFileAsBase64(_this.get(0).files[0], function(data){
							fileData = data;
							if ( fileData ) $.fn.cropOpenWindowToCrop(ipt, fileData, width, height, opts);
						});
					}
				});
			}
				...
				_this.click(function(){
					$.fn.cropOpenWindowToCrop(ipt, _this.attr('href'), width, height, opts);
					return false;
				});
			}
		}
	...
	$.fn.cropOpenWindowToCrop = function (ipt, data, width, height, opts) {
		if (data == '' || data == null || typeof(data) == 'undefined') return;
		var sourceObj = arguments[5] ? arguments[5] : null;

		$('body').append('<div id="crop-window"><div class="crop-window-wrapper"><div class="title"><h3>图片裁剪</h3><a href="javascript:;" class="close"></a></div><div class="content clearfix"><div class="big"><img id="crop-window-big" src="" /></div></div><div class="btns"><span class="tool-btn"><i class="fa fa-rotate-left"></i>向左旋转</span><span class="tool-btn"><i class="fa fa-rotate-right"></i>向右旋转</span><span class="tool-btn"><i class="fa fa-minus"></i>缩小</span><span class="tool-btn"><i class="fa fa-plus"></i>放大</span><button type="button">保存修改</button></div></div></div>');

		$('#crop-window .content').height($('#crop-window .crop-window-wrapper').height() - 28 - 62);
		$('#crop-window-big').attr('src', data);
		$('#crop-window-big').parent().css({ 'width': '100%', 'max-height': '100%' });
		$('#crop-window .close').click(function(){
			$.fn.cropOpenWindowClose(ipt, width, height, sourceObj, true, opts);
			return false;
		});
		$('#crop-window .btns button').click(function(){
			$.fn.cropOpenWindowClose(ipt, width, height, sourceObj, false, opts);
		});
		...
		$.fn.cropOpenWindowToCropChange = function(c, ipt, boundx, boundy) {
			...
			$('#crop-window').attr('bounds', $('#crop-window .jcrop-holder').width() + ',' + $('#crop-window .jcrop-holder').height() + ',' + c.x + ',' + c.y + ',' + c.x2 + ',' + c.y2 + ',' + r1 + ',' + s1);
		...
		$.fn.cropOpenWindowClose = function(ipt, width, height, sourceObj, close, opts) {
