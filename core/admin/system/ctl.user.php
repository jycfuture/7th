...
class ctl_user extends adminPage
{
	...
	public function index_action () #act_name = 用户列表#
	{
		...
		switch ($type)
		{
			case 'name'			: $where[] = "t0.name like '%$keyword%'"; break;
			case 'displayName'	: $where[] = "t0.displayName like '%$keyword%'"; break;
			case 'email'		: $where[] = "t0.email like '%$keyword%'"; break;
		}
	...
	public function add_action () #act_name = 添加用户#
	{
		...
		if (is_post())
		{
			...
			if ($data = $this->_filter($data))
			{
				...
				if ($user->addUser($data)) $this->sheader('?con=admin&ctl=system/user');
