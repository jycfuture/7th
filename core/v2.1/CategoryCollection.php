...
class CategoryCollection {
	...
	public static function getAllChildsId( $mdl, $id = 0, & $childs = [] ) {
		$data = self::instance()->data['basic'];
		if ( !$data ) {
			$data = $mdl->getList( array( 'id', 'parentId' ) );
			self::instance()->data['basic'] = $data;
		}
	 ...
	 */
	public static function getArticleCnt( $mdl, $mdlCat, & $catList, $where = array(), $refresh = false ) {
		$cntList = self::instance()->data['cntList'];
		if ( !$cntList || $refresh ) {
			$sql = "select count(*) as cnt, categoryId from ".$mdl->getTableName().self::instance()->getWhereStr( $where )." group by categoryId";
		...
		foreach ( $catList as $key => $item ) {
			foreach ( $cntList as $cnt ) {
				if ( $item['id'] == $cnt['categoryId'] ) {
					$catList[$key]['cnt'] = $cnt['cnt'];
					break;
				}
			}
			$catList[$key]['cntByChilds'] = (int)$catList[$key]['cnt'] + ( $item['id'] > 0 ? self::instance()->getArticleCntByChilds( $cntList, self::instance()->getAllChildsId( $mdlCat, $item['id'] ) ) : 0 );
		}
	}
	 ...
	 * 获取所有子分类的文章数量统计
	 */
	private static function getArticleCntByChilds( $cntList, $childs ) {
		$all = 0;
		foreach ( $cntList as $cnt ) {
			if ( in_array( $cnt['categoryId'], $childs ) ) $all += $cnt['cnt'];
		}
		return $all;
	}
