...
class db
{
	...
	function getSelectSql ($array = null, $tableName = '', $condition = null, $order = '', $limit = '')
	{
		...
		if (is_array($condition))
		{
			...
			{
				$i++;
				$whereStr	.= ($i > 1 ? ' and ' : '').(empty($key) || is_numeric($key) ? $value : '`'.$key.'`'.'=\''.( get_magic_quotes_gpc() ? $value : mysql_real_escape_string( $value ) ).'\'');
			}
		}
		else
		{
	...
	{
		if ($sql = $this->getSelectSql($array, $tableName, $condition, $order, $limit)) return $this->query($sql);
	}
	...
	function getSelectMultipleSql ($array = array(), $tableName = array(), $on = array(), $condition = null, $order = '', $limit = '')
	{
		...
		if (is_array($condition))
		{
			foreach ($condition as $key=>$value)
			{
				...
				{
					$tmp = explode('#', $key);
					$whereStr .= "$aliasPre".$tmp[0].".`".$tmp[1]."`='".( get_magic_quotes_gpc() ? $value : mysql_real_escape_string( $value ) )."'";
				}
			}
		}
		else
	...
	function getInsertSql ($array = null, $tableName = '')
	{
		...
		if (is_array($array))
		{
			foreach ($array as $key=>$value)
			{
				...
				else $valueStr	.= ($i > 1 ? ',' : '').'\''.mysql_real_escape_string($value).'\'';
			}
	...
	function getUpdateSql ($array = null, $tableName = '', $condition = null)
	{
		...
		foreach ($array as $key=>$value)
		{
			...
			else $valueStr	.= ($i > 1 ? ', ' : '').'`'.$key.'`'.'=\''.mysql_real_escape_string($value).'\'';
		}
		$i = 0;
		if (is_array($condition))
		{
			foreach ($condition as $key=>$value)
			{
				...
				else $whereStr	.= ($i > 1 ? ' and ' : '').(empty($key) || is_numeric($key) ? $value : '`'.$key.'`'."='".mysql_real_escape_string($value)."'");
			}
		}
		else
		{
	...
	public function getMin ($tb, $cell, $condition)
	{
		...
		if (is_array($condition))
		{
			...
			{
				$i++;
				$where	.= ($i > 1 ? ' and ' : '').(empty($key) || is_numeric($key) ? $value : '`'.$key.'`=\''.( get_magic_quotes_gpc() ? $value : mysql_real_escape_string( $value ) ).'\'');
			}
		}
		else
		{
